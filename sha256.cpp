...
std::string sha256(const std::string str) {
    ...
    // Pre-processing
    std::vector<uint8_t> block(str.begin(), str.end());
    uint64_t original_bit_len = block.size() * 8;
    
    // Padding: Append '1' bit
    block.push_back(0x80);
    
    // Padding: Append '0' bits until message length is congruent to 448 mod 512
    while ((block.size() * 8) % 512 != 448) {
        block.push_back(0x00);
    }
    
    // Padding: Append original length as 64-bit integer (Big Endian)
    for (int i = 7; i >= 0; i--) {
        block.push_back((original_bit_len >> (i * 8)) & 0xFF);
    }
    ...
    for (size_t i = 0; i < block.size(); i += 64) {
        uint32_t w[64];
        for (int j = 0; j < 16; j++) {
            w[j] = (block[i + j * 4] << 24) | (block[i + j * 4 + 1] << 16) | (block[i + j * 4 + 2] << 8) | (block[i + j * 4 + 3]);
        }
        for (int j = 16; j < 64; j++) {
        ...
        for (int j = 0; j < 64; j++) {
            ...
            uint32_t ch = (e & f) ^ ((~e) & g);
            uint32_t temp1 = h_temp + S1 + ch + k[j] + w[j];
